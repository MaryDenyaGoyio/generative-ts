...
class VRNN(nn.Module):
    ...
    def forward(self, x, plot=False):
        ...
        if plot:
            ...
            plt.plot(mean_x, label=r'sampled $\mu^Y_t$ with sampled $z_t$')
    ...
    def inference(self, x, plot_name = None):
        ...
        if plot_name is not None:
            ...
            plt.xlabel(r'step t')
            plt.ylabel(r'Value')
            plt.title(f'VRNN λ={self.lmbd}: ' + r'sampled $\theta_t$ given $Y_{t-1}$')
            plt.legend()
            plt.grid(True)
    ...
    def _ent_gauss(self, std):
        return self.lmbd * ( torch.log(2*math.pi*math.e*(std**2))/2 ).mean()
