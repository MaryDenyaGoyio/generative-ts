...
class VRNN(nn.Module):
    ...
    def sample(self, x, T, plot_name = None):
        ...
        with torch.no_grad():
            ...
            for t in range(T): 
                ...
                z_sample[t] = z_sample.data
                z_mean[t] = pr_mean_t.data
                z_std[t] = pr_std_t.data
    ...
    def inference(self, x, T, N=100, online = True, plot_name = None):
        ...
        for i in range(N):
            z_samples[i], z_means[i], z_stds[i] = self.sample(x)
        
        # for t in range(x.size(0)): z_sample[t] = self.sample(x[:t+1])[t] # (T * D_z)

        z_mean_avg = z_means.mean(axis=0)  # shape (T, D_z)
        z_std_avg = z_stds.mean(axis=0)    # shape (T, D_z)


        if plot_name is not None:
            plt.figure()
            plt.plot(x, label=r'$Y_t$ example')
            z_line, = plt.plot(z_mean_avg, label=r'$\theta_t | Y_{< t}, \theta_{< t}$')
            plt.fill_between(np.arange(T)[(x.size(0) if online else 0):], z_mean_avg - z_std_avg, z_mean_avg + z_std_avg, alpha=0.2, color=z_line.get_color())
            plt.xlabel(r'step t')
            plt.ylabel(r'Value')
